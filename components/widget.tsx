...
const Widget = () => {
  ...
  const animateToPosition = useCallback(
    (targetX: number, targetY: number, duration = 600) => {
...
//   );
  ...
  const snapToEdge = useCallback(() => {
    ...
    const targetY = Math.max(
      margin,
      Math.min(finalY, windowHeight - widgetSize - margin)
    );
    ...
    <>
      <div
        ...
        className={`fixed z-50 select-none transition-opacity duration-300 ${
          isHovered || isExpanded || isDragging ? "opacity-100" : "opacity-20"
        }`}
        style={{
          left: `${position.x}px`,
          top: `${position.y}px`,
      ...
      >
        {/* Widget fermé */}
        <div
          className={`w-16 h-16 bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 rounded-full shadow-2xl backdrop-blur-sm border-2 border-white/20 flex items-center justify-center transition-all duration-500 ${
            isExpanded
        ...
        {/* Widget étendu */}
        <div
          className={`absolute top-0 bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 shadow-2xl transition-all duration-500 ${
            isExpanded
              ? "scale-100 opacity-100"
              : "scale-0 opacity-0 pointer-events-none"
          }`}
          ...
          {isExpanded && (
            <div className="p-4 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <h3 className="text-white font-semibold text-sm animate-fade-in">
                  Paramètres
                </h3>
                <button
                  onClick={(e) => {
                ...
                <div className="space-y-1">
                  {languages.map((lang, index) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-2 rounded-lg transition-all duration-200 text-sm hover:scale-105 active:scale-95 ${
                        language === lang.code
              ...
              {/* Indicateur de drag */}
              <div
                className="flex justify-center pt-2 animate-fade-in"
